...
class TestBookManager(unittest.TestCase):
    ...
    def test_get_categories(self):
        ...
        result, _ = self.bm.get_categories()
        assert len(result) > 0
    ...
    def test_get_books_in_category(self):
        ...
        book_list, error = self.bm.get_books_in_category(self.category1)
    ...
    def test_get_book(self):
        ...
        book, error = self.bm.get_book(self.book1.book_id)
        assert book and not error
        self.inspect_book_info(book)
    ...
    def test_search_by_keyword(self):
        ...
        result, error = self.bm.search_by_keyword("test")
        assert not error
        assert len(result) == 1
        self.inspect_book_info(result[0])

    def test_search_similar_books(self):
        self.bm.es_manager.search_by_id.return_value = self.book1.dict()
        self.bm.es_manager.search_similar_docs.return_value = [(2, self.book2.dict(), 1.0)]
        result, error = self.bm.search_similar_books(str(self.book1.book_id))
        assert not error
        assert len(result) == 1
        self.inspect_book_info(result[0])
    ...
    def test_add_book(self):
        ...
        book_id, error = self.bm.add_book({3: self.book1.dict()})
        assert not error
        assert book_id == 3
    ...
    def test_update_book(self):
        ...
        with patch('pathlib.Path.rename'):
            result, error = self.bm.update_book(self.book1.book_id, {"category": "new_cat", "title": "new_title", "author": "new_author", "file_type": "txt"})
        assert not error
        assert result == "Ok"

    def test_get_book_not_found(self):
        self.bm.es_manager.search_by_id.return_value = None
        book, error = self.bm.get_book(999)
        self.assertIsNone(book)
        self.assertIn("No book found", error)

    @patch('pathlib.Path.rename', side_effect=IOError("Permission denied"))
    def test_update_book_io_error(self, mock_rename):
        self.bm.es_manager.search_by_id.return_value = self.book1.dict()
        result, error = self.bm.update_book(self.book1.book_id, {"category": "new_cat", "title": "new_title", "author": "new_author", "file_type": "txt"})
    ...
    def test_delete_book(self):
        ...
        with patch('pathlib.Path.unlink'):
            result, error = self.bm.delete_book(self.book1.book_id)
        assert not error
        assert result == "Ok"
