...
def clear_lastfailed_for_files(test_file_prefixes: list[str]) -> None:
    ...
    for cache_dir in candidate_cache_dirs:
        ...
        try:
            ...
            cleaned = {key: val for key, val in data.items() if not any(key == prefix or key.startswith(prefix + "::") for prefix in test_file_prefixes)}
...
def run_coverage_report() -> bool:
    ...
    for f in (coverage_file, backup_file):
        if f.exists() and not is_valid_coverage_db(f):
            try:
                f.unlink()
                print(f"⚠️  Removed corrupted {f.name}.")
            except OSError:
                pass
