...
def main() -> int:
    ...
    for arg in file_args:
        ...
        if target_path.is_file():
            ...
            file_iter = (p for p in target_path.rglob("*") if p.is_file() and not any(part.startswith(".") or part in BookManager.IGNORED_DIR_NAMES for part in p.relative_to(target_path).parts))
            ...
            if len(file_args) == 1:
                ...
                for subdir in target_path.iterdir():
                    if subdir.is_dir() and not subdir.name.startswith(".") and subdir.name not in BookManager.IGNORED_DIR_NAMES:
