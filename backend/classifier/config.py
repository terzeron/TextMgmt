...
DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    "model": {
        ...
        #
        # sklearn 기본 1000 으로는 47개 클래스가 수렴하지 못하고 경고를 냈다. 5000 으로
        # 올리면 경고가 1건으로 줄지만 성적은 소수점까지 같았다(판정률 91.7%,
        # top-1 86.5%). 미수렴이 성적을 깎고 있다는 가설은 실측으로 부정됐다.
        # 마진이 최적이 아니어도 판정 순위는 이미 자리를 잡는다.
        #
        # 그래도 기본값을 올려 둔다. 경고가 무더기로 나면 진짜 수렴 문제가 생겼을 때
        # 묻힌다. 5000 은 학습이 27% 길어져 그 사이를 택했다.
        "max_iter": 2000,
