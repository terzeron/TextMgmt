...
async def http_exception_handler(request: Request, exc: HTTPException):
    LOGGER.error(
        "[%d] %s %s - %s", exc.status_code, request.method, request.url.path, exc.detail
    )
    from fastapi.responses import JSONResponse
...
class CustomJSONResponse(JSONResponse):
    def render(self, content) -> bytes:
        return json.dumps(
            content, ensure_ascii=False, separators=(",", ":"), indent=2
        ).encode("utf-8")
...
def create_item_router(manager, content_type: str = "book") -> APIRouter:
    ...
    async def update_book(
        book_id: int, book_item: BookModel, force: bool = False
    ) -> Dict[str, Any]:
        LOGGER.debug(
            "# update_book(book_id=%d, book=%r, force=%s)", book_id, book_item, force
        )
    ...
    async def get_book_preview(book_id: int, pages: int = 5, chapters: int = 3):
        ...
        return await manager.get_book_preview(
            book_id=book_id, pages=pages, chapters=chapters
        )
    ...
    async def validate_book(book_id: int) -> Dict[str, Any]:
        ...
        if book.file_type == "epub":
            ...
            response_object["error"] = (
                f"Validation not supported for type: {book.file_type}"
            )
            return response_object
    ...
    async def rename_category(body: CategoryRenameModel) -> Dict[str, Any]:
        ...
        response_object: Dict[str, Any] = {"status": "failure"}
        result, error = await manager.rename_category(
    ...
    async def delete_category(body: CategoryDeleteModel) -> Dict[str, Any]:
        ...
        if error is None:
            ...
            hidden_list = await asyncio.to_thread(
                category_mapping.get_hidden_categories, content_type=content_type
            )
    ...
    async def search_similar_books(
        book_id: int, offset: int = 0, limit: int = 10
    ) -> Dict[str, Any]:
        LOGGER.debug(
        ...
        response_object: Dict[str, Any] = {"status": "failure"}
        similar_list, total, error = await manager.search_similar_books_paged(
            book_id, size=limit, offset=offset
        )
        if similar_list and error is None:
            response_object["status"] = "success"
            response_object["result"] = [
                BookModel(**book.dict()) for book in similar_list
            ]
    ...
    async def search_by_keyword(
        keyword: str, offset: int = 0, limit: int = 10, exclude_categories: str = ""
    ) -> Dict[str, Any]:
        LOGGER.debug(
        ...
        result, total, error = await manager.search_by_keyword_paged(
            keyword, size=limit, offset=offset, exclude_categories=excluded
        )
        if error is None:
            response_object["status"] = "success"
            response_object["result"] = [BookModel(**book.dict()) for book in result]
    ...
    async def reload_category(body: CategoryDeleteModel) -> Dict[str, Any]:
        ...
        response_object: Dict[str, Any] = {"status": "failure"}
        result, error = await manager.reload_category(
            body.category, content_type=content_type
        )
        if error is None:
            response_object["status"] = "success"
            response_object["result"] = result
    ...
    async def get_category_mismatch_details(category: str) -> Dict[str, Any]:
        ...
        try:
            result = await asyncio.to_thread(
                manager.get_category_mismatch_details, category
            )
            response_object["status"] = "success"
            response_object["result"] = result
        except Exception as e:
...
app.include_router(
    create_item_router(comics_manager, content_type="comic"), prefix="/comics"
)
...
async def search_bookstore_api(
    store_name: str, title: str = "", author: str = "", isbn: str = ""
):
    """
    ...
    results, search_keyword, search_method = await asyncio.to_thread(
        bookstore.search, isbn=isbn, title=title, author=author
    )
    ...
    if not books_data:
        return {
            ...
            "search_url": bookstore.build_search_url(search_keyword)
            if search_keyword
            else "",
            "result": [],
        }
...
async def get_all_category_mappings(content_type: str = "book") -> Dict[str, Any]:
    ...
    try:
        mappings = await asyncio.to_thread(
            category_mapping.get_all_mappings, content_type=content_type
        )
...
async def get_category_keywords(
    category: str, content_type: str = "book"
) -> Dict[str, Any]:
    """특정 카테고리의 키워드 목록 조회"""
    LOGGER.debug(
    ...
    try:
        keywords = await asyncio.to_thread(
            category_mapping.get_keywords, category, content_type=content_type
        )
...
async def set_category_keywords(
    category: str, body: CategoryKeywordsModel, content_type: str = "book"
) -> Dict[str, Any]:
...
@app.post(
    "/category-mappings/{category}/keywords", dependencies=[Depends(require_admin)]
)
async def add_category_keyword(
    category: str, body: Dict[str, str], content_type: str = "book"
) -> Dict[str, Any]:
    """카테고리에 키워드 추가"""
    ...
    try:
        success = await asyncio.to_thread(
            category_mapping.add_keyword, category, keyword, content_type=content_type
        )
        if success:
            return {
                "status": "success",
...
async def remove_category_keyword(
    category: str, keyword: str, content_type: str = "book"
) -> Dict[str, Any]:
    """카테고리에서 키워드 삭제"""
    LOGGER.debug(
...
async def delete_category_mapping(
    ...
    try:
        success = await asyncio.to_thread(
            category_mapping.delete_category, category, content_type=content_type
        )
        if success:
...
async def update_all_category_mappings(
    body: CategoryMappingsModel, content_type: str = "book"
) -> Dict[str, Any]:
    """전체 매핑 일괄 업데이트"""
...
async def set_hidden_category(
    category: str, body: HiddenCategoryModel, content_type: str = "book"
) -> Dict[str, Any]:
    """카테고리 비노출 설정/해제"""
    LOGGER.debug(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=500, detail="Failed to update hidden category"
            )
    except HTTPException:
        raise
    except Exception as e:
