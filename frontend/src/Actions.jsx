/* eslint-disable react-refresh/only-export-components --
   유사도/문자열 처리 유틸들이 동일 도메인 응집을 위해 컴포넌트와 co-located. HMR 힌트일 뿐 런타임 영향 없음. */
...
export const stripNoiseWords = (keywords) => {
  return keywords
    .map((kw) =>
      NOISE_PATTERNS.reduce((s, pat) => s.replace(pat, ""), kw).trim(),
    )
    .filter(Boolean);
};
...
const calculateCategoryScores = (bookstoreCategory, categoryList) => {
  ...
  for (const category of categoryList) {
    ...
    const dirKeywords = stripNoiseWords([
      ...(mappings[category] || []),
      categoryName,
    ]);
...
const findTopSimilarCategories = (
  suggestedCategories,
  categoryList,
  topN = 3,
) => {
  ...
  for (const [_store, bookstoreCategory] of Object.entries(
    suggestedCategories,
  )) {
    if (!bookstoreCategory) continue;

    const storeScores = calculateCategoryScores(
      bookstoreCategory,
      categoryList,
    );
...
export const getSimilarityDebugInfo = (
  suggestedCategories,
  categoryList,
  topN = 10,
) => {
  ...
  for (const [store, bookstoreCategory] of Object.entries(
    suggestedCategories,
  )) {
    if (!bookstoreCategory) continue;
    const rawKeywords = splitBySpecialChars(bookstoreCategory);
  ...
  for (const category of categoryList) {
    ...
    for (const [store, storeInfo] of Object.entries(
      debugInfo.bookstoreKeywords,
    )) {
      ...
      for (const deepKeyword of storeInfo.keywords) {
        ...
        for (const dirKeyword of dirKeywords) {
          ...
          if (similarity > bestSimilarity) {
            ...
            const isContained =
              drLower.includes(dkLower) || dkLower.includes(drLower);
            bestMatch = {
...
export default function Actions(props) {
  ...
  const highlightedCategories = useMemo(() => {
    if (
      ...
      Object.keys(props.suggestedCategories).length > 0 &&
      props.otherCategoryList?.length &&
      mappingsLoaded
    ) {
      const filteredCategoryList = props.otherCategoryList.filter(
        (cat) => cat !== "_root",
      );
      return findTopSimilarCategories(
        props.suggestedCategories,
  ...
  useEffect(() => {
    if (
      ...
      props.selectDirectoryButtonClicked
    ) {
      setManuallyClicked(false);
      props.selectDirectoryButtonClicked(null, highlightedCategories[0]);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps -- props 전체를 deps에 넣으면 무관한 prop 변경 시에도 재실행되므로 사용 필드만 명시 (의도된 누락)
      ...
      ?.map((category) => {
        ...
        if (hasSubCategory) {
          ...
          return {
            ...
            label: subCategory,
            style: {
              backgroundColor: getRandomLightColor(prefix),
              color: "#333",
            },
          };
        }
      ...
      <Row className="button_group">
        ...
        {!props.selectedEntryId.startsWith(ROOT_DIRECTORY) > 0 && (
          <Button
            variant="outline-warning"
            className="btn-xs"
            onClick={props.moveToUpperButtonClicked}
            disabled={!props.newFileName || props.isProcessing}
          >
            상위로
            <FontAwesomeIcon
              icon={props.isProcessing ? faSpinner : faUpload}
        ...
        {renderingInfoList.map((info) => {
          ...
          const keepHighlight =
            isSelected && !manuallyClicked && highlightClass;
