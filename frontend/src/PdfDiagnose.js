...
async function checkPdfParsing(buffer) {
    ...
    try {
        ...
        if (msg.includes("password")) {
            results.push({type: "error", severity: "ERROR", text: `암호화된 PDF — 비밀번호 필요: ${msg}`});
        } else {
            results.push({type: "error", severity: "FATAL", text: `pdf.js 파싱 실패: ${msg}`});
        }
...
async function checkMetadata(pdfDoc) {
    ...
    try {
        ...
        if (found === 0) {
            results.push({type: "warn", severity: "WARNING", text: "문서 메타데이터 없음"});
        } else {
            results.push({type: "ok", text: `메타데이터 필드 ${found}개 확인`});
        }
    } catch (e) {
        results.push({type: "error", severity: "ERROR", text: `메타데이터 추출 실패: ${e.message}`});
    }
...
async function checkPageStructure(pdfDoc) {
    ...
    try {
        ...
        const w = x2 - x1;
        const h = y2 - y1;
        results.push({type: "info", text: `첫 페이지 크기: ${Math.round(w)} × ${Math.round(h)} pt`});
    } catch (e) {
        results.push({type: "error", severity: "ERROR", text: `첫 페이지 로드 실패: ${e.message}`});
    }
    ...
    for (const idx of sampleIndices) {
        try {
            ...
            failCount++;
            results.push({type: "error", severity: "ERROR", text: `페이지 ${idx} 로드 실패: ${e.message}`});
        }
    }

    if (failCount === 0) {
        results.push({type: "ok", text: `샘플 ${sampleIndices.length}개 페이지 접근 정상`});
    }
...
async function checkTextExtraction(pdfDoc) {
    ...
    try {
        ...
        if (textLength > 0) {
            results.push({type: "ok", text: `첫 페이지 텍스트 추출: ${textLength}자`});
        } else {
            results.push({type: "warn", severity: "WARNING", text: "첫 페이지에서 텍스트 추출 불가 (이미지 기반 PDF일 수 있음)"});
        }
    } catch (e) {
        results.push({type: "error", severity: "ERROR", text: `텍스트 추출 실패: ${e.message}`});
    }
