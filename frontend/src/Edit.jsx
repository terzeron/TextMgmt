...
export default function Edit({ basePath = "/book-edit", apiPrefix = "" }) {
  ...
  const decomposeTitle = useCallback((book) => {
    ...
    if (book["author"] !== "") {
      ...
      // [ 저자 ] 제목 . 확장자
      const pattern1 =
        /^\s*\[(?<author>.*?)]\s*(?<title>.*?)\s*\.(?<extension>\w+)\s*$/;
      // ( 저자 ) 제목 . 확장자
      const pattern2 =
        /^\s*\((?<author>.*?)\)\s*(?<title>.*?)\s*\.(?<extension>\w+)\s*$/;
      // 제목 [ 저자 ] . 확장자
      const pattern3 =
        /^\s*(?<title>.*?)\s*\[\s*(?<author>.*?)\s*]\s*\.(?<extension>\w+)\s*$/;
      // 제목 @ 저자 . 확장자
      const pattern4 =
        /^\s*(?<title>.*?)\s*@\s*(?<author>.*?)\s*\.(?<extension>\w+)\s*$/;
      // 저자 - 제목 . 확장자 or 저자 _ 제목 . 확장자
      const pattern5 =
        /^\s*(?<author>.*?)\s*[_-]\s*(?<title>.*?)\s*\.(?<extension>\w+)\s*$/;
      // 제목 ( 저자 )
      const pattern6 =
        /^\s*(?<title>.*?)\s*\(\s*(?<author>.*?)\s*\)\s*\.(?<extension>\w+)\s*$/;
      // 모두 제목으로 간주
      ...
      if (selectedFolderData && selectedFolderData.fileType === "folder") {
        ...
        if (booksInCategory) {
          ...
          if (book) {
            ...
            // determine nextEntryId / prevEntryId
            const nextEntryId = determineNextEntryId(
              folderData,
              selectedEntryId,
            );
            setNextEntryId(nextEntryId);
            nextEntryIdRef.current = nextEntryId;
            const prevEntryId = determinePrevEntryId(
              folderData,
              selectedEntryId,
            );
            setPrevEntryId(prevEntryId);
  ...
  useEffect(() => {
    ...
    if (
      ...
      const firstFile = folderData.find(
        (item) => item.fileType !== "folder" && item.book,
      );
      if (firstFile) {
  ...
  const checkEntryExistence = useCallback(
    (folderData, newDirName, newFileName) => {
      console.log(
        `checkEntryExistence(newDirName=${newDirName}, newFileName=${newFileName})`,
      );
  ...
  const removeEntryFromFolderData = useCallback(
    (folderData, dirName, fileName) => {
      ...
      if (dirName === "_root" || dirName === "") {
        ...
        console.log(
          `removeEntryFromFolderData(): removed root file ${entryId}`,
        );
      } else {
        // 2단계 트리에서 폴더 내 파일 제거
        const entryId = dirName + "/" + fileName;
        newFolderData = updateFolderChildren(
          newFolderData,
          dirName,
          (children) => children.filter((item) => item.id !== entryId),
        );
  ...
  const appendEntryToFolderData = useCallback(
    (folderData, newDirName, newFileName) => {
      let newFolderData = [...folderData];
      ...
      if (isRootFile) {
        // 최상위 파일 추가 (폴더들 뒤에 추가)
        const folderEndIndex = newFolderData.findIndex(
        ...
        console.log(
          `appendEntryToFolderData(): pushed ${newFileName} to ${newDirName}`,
        );
      }
      return newFolderData;
    },
  ...
  const updateFile = useCallback(
    (dirName, fileName, newDirName, newFileName, force = false) => {
      if (isProcessingRef.current) return;

      const baseUrl = apiPrefix + "/books/" + bookInfo["book_id"];
      // 백엔드는 '_root'를 기대하므로 빈 문자열을 '_root'로 변환
      const categoryForBackend =
        newDirName === "" || newDirName === "_root" ? "_root" : newDirName;
      ...
      if (
        ...
        if (
          !window.confirm(
            `"${displayNewDirName}/${newFileName}"이(가) 이미 존재합니다.\n기존 파일을 덮어쓰시겠습니까?`,
          )
        ) {
          return;
        }
      ...
      console.log(
        `updateFile: PUT ${requestUrl}, payload.title="${payload.title}", payload.author="${payload.author}", payload.file_path="${payload.file_path}"`,
      );
      ...
      const handleSuccess = () => {
        ...
        const displayDirName =
          dirName === "" || dirName === "_root" ? "최상위" : dirName;
        const displayNewDirName =
          newDirName === "" || newDirName === "_root" ? "최상위" : newDirName;
        let message;
        ...
        newFolderData = appendEntryToFolderData(
          newFolderData,
          newDirName,
          newFileName,
        );
      ...
      jsonPutReq(requestUrl, payload, handleSuccess, (error) => {
        ...
        if (
          ...
          if (
            window.confirm(
              error.substring("CONFLICT:".length) +
                "\n기존 파일을 덮어쓰시겠습니까?",
            )
          ) {
            jsonPutReq(
    ...
    if (selectedEntryId?.includes("/")) {
      ...
      jsonDeleteReq(
        ...
        (response) => {
          ...
          const newFolderData = removeEntryFromFolderData(
            folderData,
            dirName,
            bookId,
          );
          ...
          if (nextEntryIdRef.current && entryClickedRef.current) {
            console.log(
              `deleteButtonClicked(): nextEntryId=${nextEntryIdRef.current}`,
            );
            entryClickedRef.current(nextEntryIdRef.current);
          } else if (prevEntryIdRef.current && entryClickedRef.current) {
            console.log(
              `deleteButtonClicked(): prevEntryId=${prevEntryIdRef.current}`,
            );
            entryClickedRef.current(prevEntryIdRef.current);
          } else {
            setSuccessMessage(message + " (마지막 책이었습니다.)");
              ...
              <Row id="top_panel">
                <Col id="left_panel" md="6" lg="5" className="ps-0 pe-0">
                  <Card>
                    ...
                    <Suspense
                      fallback={<div className="loading">로딩 중...</div>}
                    >
                      <Card.Body>
                        <BookInfoView
                          bookInfo={bookInfo}
                          ...
                          onCutTitleButtonClick={cutTitleButtonClicked}
                          onCutAuthorButtonClick={cutAuthorButtonClicked}
                          onExchangeButtonClick={exchangeButtonClicked}
                          onResetButtonClick={resetButtonClicked}
                          newFileName={newFileName}
                          newFileNameChanged={newFileNameChanged}
                            ...
                            <Col>
                              <InputGroup>
                                ...
                                <Button
                                  ...
                                  disabled={!selectedEntryId || isProcessing}
                                >
                                  변경
                                  <FontAwesomeIcon
                          ...
                          <Row className="button_group">
                            <Col>
                              <Actions
                                ...
                                toNextEntryClicked={toNextEntryButtonClicked}
                                toPrevEntryClicked={toPrevEntryButtonClicked}
