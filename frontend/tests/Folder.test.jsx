...
describe("Folder", () => {
  ...
  it("'더 보기' 항목 클릭 시 onClickHandler만 호출하고 디렉토리를 접지 않는다", () => {
    const onClickHandler = vi.fn();
    const onToggle = vi.fn();
    const data = [
      {
    ...
    fireEvent.click(screen.getByText("더 보기 (1/10)"));

    // 다음 페이지 요청은 전달되어야 한다
    expect(onClickHandler).toHaveBeenCalledWith("1_fiction/__more__");
    // 책 선택이 아니므로 디렉토리 패널이 닫히면 안 된다
    expect(onToggle).not.toHaveBeenCalled();
  });

  it("'더 보기' 항목 클릭은 expandedItems를 변경하지 않는다", () => {
