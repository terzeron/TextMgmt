...
describe('ViewEPUB', () => {
    ...
    it('30초 내 로딩 미완료 시 타임아웃 에러를 표시한다', async () => {
        ...
        expect(screen.getByText(/EPUB 렌더링 시간이 초과되었습니다\. \(book_id=1\)/)).toBeTruthy();
    ...
    it('글꼴 변경 시 rendition.themes.font가 호출된다', async () => {
        ...
        await act(async () => {
            fireEvent.change(select, { target: { value: "'Nanum Gothic', sans-serif" } });
        });

        expect(rendition.themes.font).toHaveBeenCalledWith("'Nanum Gothic', sans-serif");
    });
    ...
    it('전체보기에서 페이지 변경 시 위치를 localStorage에 저장한다', async () => {
        ...
        expect(localStorageMock.setItem).toHaveBeenCalledWith('epub_location_42', 'epubcfi(/6)');
    });
    ...
    it('전체보기에서 첫 렌더 후 저장된 위치로 display() 복원한다', async () => {
        localStorageMock._getStore()['epub_location_42'] = 'epubcfi(/saved/location)';
        autoLoad = false;
        render(<ViewEPUB bookId={42} />);
    ...
    it('미리보기에서는 getRendition이 relocated 리스너를 등록하지 않는다', async () => {
        ...
        expect(rendition.on).not.toHaveBeenCalledWith('relocated', expect.any(Function));
    });
    ...
    it('locations.generate 실패 시 페이지 계산 중 상태가 유지된다', async () => {
        ...
        const rendition = createMockRendition();
        rendition.book.locations.generate = vi.fn(() => Promise.reject(new Error('generate failed')));
    ...
    it('서버 에러 시 응답 본문을 에러 메시지에 표시한다', async () => {
        ...
        render(<ViewEPUB bookId={1} />);
        await waitFor(() => {
            expect(screen.getByText(/EPUB 로딩 실패: Custom error body/)).toBeTruthy();
        });
    });
    ...
    it('서버 에러 본문이 비어있으면 status 코드를 표시한다', async () => {
        ...
        render(<ViewEPUB bookId={1} />);
        await waitFor(() => {
            expect(screen.getByText(/EPUB 로딩 실패: 서버 응답 오류: 404/)).toBeTruthy();
        });
    });
    ...
    it('book.ready reject 시 에러 메시지를 표시한다', async () => {
        ...
        const rendition = createMockRendition({ readyReject: new Error('Book parsing failed') });
        await act(async () => {
            capturedGetRendition(rendition);
        });

        await waitFor(() => {
            expect(screen.getByText(/EPUB 파싱 오류: Book parsing failed/)).toBeTruthy();
        });
    });
    ...
    it('displayerror 이벤트 발생 시 에러 메시지를 표시한다', async () => {
        ...
        await act(async () => {
            rendition._handlers['displayerror'](new Error('Display rendering failed'));
        });

        await waitFor(() => {
            expect(screen.getByText(/EPUB 렌더링 오류: Display rendering failed/)).toBeTruthy();
        });
    });
    ...
    it('book.ready reject 시 로딩 타임아웃이 취소된다', async () => {
        ...
        const rendition = createMockRendition({ readyReject: new Error('Book load failed') });
        await act(async () => {
            capturedGetRendition(rendition);
        });
    ...
    it('미리보기에서도 displayerror가 에러를 표시한다', async () => {
        ...
        await waitFor(() => {
            expect(screen.getByText(/EPUB 렌더링 오류: Preview display error/)).toBeTruthy();
        });
    });
    ...
    it('미리보기에서도 book.ready reject가 에러를 표시한다', async () => {
        ...
        const rendition = createMockRendition({ readyReject: new Error('Preview parsing failed') });
        await act(async () => {
            capturedGetRendition(rendition);
        });

        await waitFor(() => {
            expect(screen.getByText(/EPUB 파싱 오류: Preview parsing failed/)).toBeTruthy();
        });
    });
    ...
    it('displayerror에 문자열이 전달되면 String()으로 표시한다', async () => {
        ...
        await waitFor(() => {
            expect(screen.getByText(/EPUB 렌더링 오류: String error message/)).toBeTruthy();
        });
    });
    ...
    it('전체보기에서 displayerror 리스너가 등록된다', async () => {
        ...
        expect(rendition.on).toHaveBeenCalledWith('displayerror', expect.any(Function));
    });
    ...
    it('rendition.display가 reject되면 에러 메시지를 표시하고 에러를 재전파한다', async () => {
        ...
        const rendition = createMockRendition();
        rendition.display = vi.fn(() => Promise.reject(new Error('Display failed')));
