...
function ProblemChild({ shouldThrow = false, errorMessage = "Boom!" }) {
  if (shouldThrow) {
...
describe("ErrorBoundary", () => {
  ...
  beforeEach(() => {
    reportSpy = vi.spyOn(clientLogger, "reportClientError").mockReturnValue(true);
  ...
  it("상세 정보 버튼 클릭 시 에러 메시지와 스택을 토글한다", () => {
    ...
    expect(screen.getAllByText(/Secret crash message/).length).toBeGreaterThan(0);
