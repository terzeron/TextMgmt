...
const sampleResults = [
  {
    book_id: 1,
    category: "소설",
    file_path: "/books/novel1.epub",
    file_type: "epub",
  },
  {
    book_id: 2,
    category: "역사",
    file_path: "/books/history/ancient.pdf",
    file_type: "pdf",
  },
];
...
describe("SearchResult", () => {
    ...
    const rootResults = [
      {
        book_id: 3,
        category: "_root",
        file_path: "/books/root_novel.epub",
        file_type: "epub",
      },
      {
        book_id: 4,
        category: "",
        file_path: "/books/empty_cat.epub",
        file_type: "epub",
      },
    ];
  ...
  it('file_path와 title이 모두 없으면 "Unknown"을 표시한다', () => {
    const noFileResults = [{ book_id: 5, category: "기타" }];
    render(<SearchResult results={noFileResults} />);
    expect(screen.getByText("기타/Unknown")).toBeTruthy();
  });

  it("file_path가 없고 title이 있으면 title을 표시한다", () => {
    const titleOnlyResults = [
      { book_id: 6, category: "기타", title: "제목만 있는 책" },
    ];
    render(<SearchResult results={titleOnlyResults} />);
    expect(screen.getByText("기타/제목만 있는 책")).toBeTruthy();
  });

  it("file_type이 없으면 기본값 epub으로 전체보기를 연다", () => {
    const mockOpen = vi.fn();
    vi.stubGlobal("open", mockOpen);
    const noFileTypeResults = [
      { book_id: 7, category: "기타", file_path: "/books/notype.bin" },
    ];
    render(<SearchResult results={noFileTypeResults} />);
    fireEvent.click(screen.getByText("전체보기"));
    expect(mockOpen).toHaveBeenCalledWith(
      expect.stringContaining("/viewer/epub/7?"),
      "_blank",
      "noopener",
    );
    vi.unstubAllGlobals();
  ...
  it("basePath가 빈 문자열이면 기본 basePath로 대체한다", () => {
    const mockOpen = vi.fn();
    vi.stubGlobal("open", mockOpen);
    render(
      <SearchResult
