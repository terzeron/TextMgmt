...
const {
  ...
  mockGetMetadata,
  mockGetPage,
  mockGetTextContent: _mockGetTextContent,
  mockDestroy,
} = vi.hoisted(() => ({
...
describe("diagnosePdf", () => {
  ...
  it("AcroForm 포함 시 info를 반환한다", async () => {
    ...
    const result = await diagnosePdf(new ArrayBuffer(8));
    const metaSection = result.sections.find((s) => s.name === "메타데이터");
    expect(metaSection.results.some((r) => r.text.includes("AcroForm"))).toBe(
      true,
    );
  });
  ...
  it("5페이지 이하에서 모든 페이지를 샘플링한다", async () => {
    ...
    const pageSection = result.sections.find((s) => s.name === "페이지 구조");
    expect(pageSection.results.some((r) => r.text.includes("샘플 3개"))).toBe(
      true,
    );
  });
  ...
  it("6페이지 이상에서 5개 샘플 페이지를 검사한다", async () => {
    ...
    const pageSection = result.sections.find((s) => s.name === "페이지 구조");
    expect(pageSection.results.some((r) => r.text.includes("샘플 5개"))).toBe(
      true,
    );
  });
