export default class SummaryReporter {
  ...
  onTestRunEnd(_testModules, unhandledErrors) {
    ...
    if (this.#failedFiles > 0) {
      write(
        ` Test Files  \x1b[31m${this.#failedFiles} failed\x1b[0m | \x1b[32m${this.#passedFiles} passed\x1b[0m (${this.#testFiles})`,
      );
    } else {
      write(
        ` Test Files  \x1b[32m${this.#passedFiles} passed\x1b[0m (${this.#testFiles})`,
      );
    }
    if (this.#failedTests > 0) {
      write(
        `      Tests  \x1b[31m${this.#failedTests} failed\x1b[0m | \x1b[32m${this.#passedTests} passed\x1b[0m (${this.#totalTests})`,
      );
    } else {
      write(
        `      Tests  \x1b[32m${this.#passedTests} passed\x1b[0m (${this.#totalTests})`,
      );
    }
    if (unhandledErrors?.length) {
      ...
      for (const error of unhandledErrors) {
        process.stderr.write(
          `   \x1b[31mUnhandled:\x1b[0m ${error.message ?? error}\n`,
        );
      }
    }
  }
