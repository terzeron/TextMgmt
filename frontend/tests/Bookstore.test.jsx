        ...
        rawJsonGetReq.mockImplementation((url, onSuccess) => {
            if (url.includes('isbn=')) {
                setTimeout(() => onSuccess({
                    status: 'success',
                    result: [{ title: 'T', author: 'A', category: '소설 > 한국소설', book_url: 'u' }]
                }), 0);
            } else {
                setTimeout(() => onSuccess({ status: 'not_found', result: [] }), 0);
            }
...
describe('Bookstore 탭 렌더링 및 버튼 클릭', () => {
    ...
    it('ISBN/저자+제목 검색 버튼이 렌더링된다', async () => {
        ...
        const authorTitleButtons = screen.getAllByRole('button', { name: '저자+제목' });
    ...
    it('ISBN 버튼 클릭 시 fetchWithMethod가 실행된다', async () => {
        rawJsonGetReq.mockImplementation((url, onSuccess) => {
            setTimeout(() => onSuccess({
                status: 'success',
                result: [{ title: 'T', author: 'A', category: '소설 > 한국소설', book_url: 'u', isbn: '978' }],
                search_url: 'https://search.example.com'
            }), 0);
        });

        await act(async () => {
            render(<Bookstore bookInfo={{ title: '제목', author: '저자', isbn: '978-123' }} />);
        });
    ...
    it('저자+제목 버튼 클릭 시 title/author 파라미터로 검색한다', async () => {
        ...
        const titleAuthorButtons = screen.getAllByRole('button', { name: '저자+제목' });
        await act(async () => {
    ...
    it('검색 결과가 있으면 제목/카테고리를 표시한다', async () => {
        rawJsonGetReq.mockImplementation((url, onSuccess) => {
            setTimeout(() => onSuccess({
                status: 'success',
                result: [
                    { title: '소설1', author: '작가A', category: '한국소설', book_url: 'http://book1', isbn: '111' },
                    { title: '소설2', author: '작가B', category: '외국소설', book_url: 'http://book2' },
                ]
            }), 0);
        });

        await act(async () => {
            render(<Bookstore bookInfo={{ title: '제목', author: '저자', isbn: '111' }} searchTrigger={1} onCategoriesFound={vi.fn()} />);
        });
    ...
    it('검색 결과가 없으면 빈 결과 메시지를 표시한다', async () => {
        ...
        await act(async () => {
            render(<Bookstore bookInfo={{ title: '제목', author: '저자', isbn: '' }} searchTrigger={1} onCategoriesFound={vi.fn()} />);
        });
    ...
    it('서점에서 보기 링크가 search_url이 있을 때 표시된다', async () => {
        ...
        await waitFor(() => {
            expect(screen.getAllByText('서점에서 보기').length).toBeGreaterThanOrEqual(1);
        });
    });
    ...
    it('fetchWithMethod 성공 시 onCategoriesFound를 호출한다', async () => {
        ...
        await act(async () => {
            render(<Bookstore bookInfo={{ title: '제목', author: '저자', isbn: '978' }} onCategoriesFound={onCategoriesFound} />);
        });
