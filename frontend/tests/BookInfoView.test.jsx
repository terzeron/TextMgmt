...
describe('BookInfoView', () => {
    ...
    it('pageCount가 0이고 lineCount가 있으면 행 수를 표시한다', () => {
        render(<BookInfoView bookInfo={{ ...defaultBookInfo, page_count: 0, line_count: 500 }} />);
        expect(screen.getByDisplayValue('500행')).toBeTruthy();
    });

    it('pageCount와 lineCount가 모두 0이면 -를 표시한다', () => {
        render(<BookInfoView bookInfo={{ ...defaultBookInfo, page_count: 0, line_count: 0 }} />);
    ...
    it('isEditEnabled=true일 때 저자/제목 입력 필드를 표시한다', () => {
        render(<BookInfoView bookInfo={defaultBookInfo} isEditEnabled={true} onAuthorChange={vi.fn()} onTitleChange={vi.fn()} />);
    ...
    it('저자 입력 변경 시 콜백을 호출한다', () => {
        ...
        render(<BookInfoView bookInfo={defaultBookInfo} isEditEnabled={true} onAuthorChange={onAuthorChange} onTitleChange={vi.fn()} />);
        fireEvent.change(screen.getByDisplayValue('테스트 저자'), { target: { value: '새 저자' } });
        expect(onAuthorChange).toHaveBeenCalled();
    });

    it('제목 입력 변경 시 콜백을 호출한다', () => {
        const onTitleChange = vi.fn();
        render(<BookInfoView bookInfo={defaultBookInfo} isEditEnabled={true} onAuthorChange={vi.fn()} onTitleChange={onTitleChange} />);
        fireEvent.change(screen.getByDisplayValue('테스트 제목'), { target: { value: '새 제목' } });
