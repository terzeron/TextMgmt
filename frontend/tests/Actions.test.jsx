...
describe("filterSubstringKeywords", () => {
  it("다른 키워드에 포함되는 짧은 키워드를 제거한다", () => {
    expect(filterSubstringKeywords(["소설", "세계각국소설"])).toEqual([
      "세계각국소설",
    ]);
  });
  ...
  it("여러 키워드 중 포함되는 것만 제거한다", () => {
    expect(filterSubstringKeywords(["소설", "세계각국소설", "역사"])).toEqual([
      "세계각국소설",
      "역사",
    ]);
  });
...
describe("stripNoiseWords", () => {
  ...
  it('여러 키워드에서 "일반"을 제거한다', () => {
    expect(stripNoiseWords(["중문일반", "영문일반", "교육일반"])).toEqual([
      "중문",
      "영문",
      "교육",
    ]);
  ...
  it('혼합: "일반" 포함과 미포함 키워드를 올바르게 처리한다', () => {
    expect(stripNoiseWords(["경영일반", "소설", "건강일반"])).toEqual([
      "경영",
      "소설",
      "건강",
    ]);
...
describe("isProcessing 버튼 비활성화", () => {
  ...
  it('isProcessing=true이면 "로 옮기기" 버튼이 비활성화된다', () => {
    render(
      <Actions {...defaultProps} selectedCategory="소설" isProcessing={true} />,
    );
    const btn = screen.getByText(
      (content, element) =>
        element.tagName === "BUTTON" &&
        element.textContent.includes("로 옮기기"),
    );
    expect(btn.disabled).toBe(true);
  });
  ...
  it('isProcessing=true일 때 "다음 책으로" 클릭해도 핸들러가 호출되지 않는다', () => {
    const fn = vi.fn();
    render(
      <Actions {...defaultProps} toNextEntryClicked={fn} isProcessing={true} />,
    );
  ...
  it('isProcessing=true일 때 "로 옮기기" 클릭해도 핸들러가 호출되지 않는다', () => {
    ...
    render(
      <Actions
        {...defaultProps}
        selectedCategory="소설"
        moveToDirectoryButtonClicked={fn}
        isProcessing={true}
      />,
    );
    const btn = screen.getByText(
...
describe("Actions 추가 분기", () => {
  it("캐시가 초기화되지 않았으면 fetchCategoryMappings를 호출한다", async () => {
    const fetchSpy = vi
      ...
      .mockResolvedValue();
    const initSpy = vi
      .spyOn(categoryMappingCache, "isCacheInitialized")
  ...
  it("언더스코어가 있는 카테고리는 접두사 색상과 하위 라벨로 렌더링한다", () => {
    render(
      <Actions {...defaultProps} otherCategoryList={["10_history_modern"]} />,
    );
...
describe("getSimilarityDebugInfo", () => {
  ...
  it("빈 카테고리 값은 건너뛴다", () => {
    const result = getSimilarityDebugInfo({ yes24: "", aladin: "한국소설" }, [
      "소설",
    ]);
...
describe("getSimilarityDebugInfo - 숫자 prefix / 빈 서점 카테고리", () => {
  it("숫자 prefix 가 있는 디렉토리명은 prefix 를 제거하고 비교한다", () => {
    const info = getSimilarityDebugInfo({ yes24: "소설" }, [
      "4_소설",
      "5_역사",
    ]);
  ...
  it("빈 서점 카테고리는 건너뛴다", () => {
    const info = getSimilarityDebugInfo({ yes24: "", aladin: "소설" }, [
      "소설",
      "역사",
    ]);
...
describe("Actions renderingInfoList 폴백", () => {
  it("otherCategoryList가 없으면 카테고리 버튼을 렌더링하지 않는다", () => {
    render(<Actions {...defaultProps} otherCategoryList={null} />);
    // infoList 계산이 undefined가 되어 빈 배열로 폴백되고, 카테고리 버튼이 렌더링되지 않음
