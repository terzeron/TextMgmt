...
vi.mock('../src/Actions', () => ({
    default: ({ toNextEntryClicked, toPrevEntryClicked, moveToUpperButtonClicked, moveToDirectoryButtonClicked, selectDirectoryButtonClicked }) => (
        <div data-testid="actions">
            ...
            <button data-testid="select-dir" onClick={(e) => selectDirectoryButtonClicked(e, '2_science')}>디렉토리선택</button>
        </div>
    ),
}));
...
describe('Edit', () => {
    ...
    it('이전 버튼 클릭 시 이전 책으로 이동한다', async () => {
        ...
        await waitFor(() => { expect(screen.getByTestId('folder-open')).toBeTruthy(); });
        fireEvent.click(screen.getByTestId('folder-item-1_fiction'));
        await waitFor(() => { expect(screen.getByText('[작가B] 소설2.pdf')).toBeTruthy(); });
        fireEvent.click(screen.getByTestId('folder-item-1_fiction/102'));
        await waitFor(() => { expect(screen.getByTestId('book-title').textContent).toBe('소설2'); });
    ...
    it('초기화 버튼 클릭 시 원래 분해된 값으로 복원한다', async () => {
        ...
        // 제목을 변경한 후
        fireEvent.change(screen.getByTestId('title-input'), { target: { value: '변경된제목' } });
        await waitFor(() => { expect(screen.getByTestId('book-title').textContent).toBe('변경된제목'); });
    ...
    it('저자자르기 버튼 클릭 시 저자를 공백 기준으로 분해한다', async () => {
        ...
        fireEvent.change(screen.getByTestId('author-input'), { target: { value: '성 이름' } });
    ...
    it('파일명 입력을 직접 변경할 수 있다', async () => {
        ...
        fireEvent.change(screen.getByTestId('filename-input'), { target: { value: '새파일명.epub' } });
        await waitFor(() => {
    ...
    it('author가 있고 title에 [author] prefix가 포함된 경우 제거한다', async () => {
        const books = [{ book_id: 401, title: '[작가E] 소설5', author: '작가E', file_type: 'epub', file_path: '1_fiction/[작가E] 소설5.epub', category: '1_fiction' }];
        setupMockCategories(CATEGORIES, books);
        render(<Edit />);
        await waitFor(() => { expect(screen.getByTestId('folder-open')).toBeTruthy(); });
        fireEvent.click(screen.getByTestId('folder-item-1_fiction'));
    ...
    it('저자 - 제목 패턴의 파일명을 분해한다', async () => {
        const books = [{ book_id: 501, title: '작가F - 소설6', author: '', file_type: 'epub', file_path: '1_fiction/작가F - 소설6.epub', category: '1_fiction' }];
        setupMockCategories(CATEGORIES, books);
        render(<Edit />);
        await waitFor(() => { expect(screen.getByTestId('folder-open')).toBeTruthy(); });
        fireEvent.click(screen.getByTestId('folder-item-1_fiction'));
    ...
    it('제목 [저자] 패턴의 파일명을 분해한다', async () => {
        const books = [{ book_id: 502, title: '소설7 [작가G]', author: '', file_type: 'epub', file_path: '1_fiction/소설7 [작가G].epub', category: '1_fiction' }];
        setupMockCategories(CATEGORIES, books);
        render(<Edit />);
        await waitFor(() => { expect(screen.getByTestId('folder-open')).toBeTruthy(); });
        fireEvent.click(screen.getByTestId('folder-item-1_fiction'));
    ...
    it('제목 (저자) 패턴의 파일명을 분해한다', async () => {
        const books = [{ book_id: 503, title: '소설8 (작가H)', author: '', file_type: 'epub', file_path: '1_fiction/소설8 (작가H).epub', category: '1_fiction' }];
        setupMockCategories(CATEGORIES, books);
        render(<Edit />);
        await waitFor(() => { expect(screen.getByTestId('folder-open')).toBeTruthy(); });
        fireEvent.click(screen.getByTestId('folder-item-1_fiction'));
    ...
    it('변경 버튼으로 같은 디렉토리 내 이름 변경 시 성공 메시지를 표시한다', async () => {
        ...
        fireEvent.change(screen.getByTestId('title-input'), { target: { value: '새제목' } });
        await waitFor(() => { expect(screen.getByTestId('book-title').textContent).toBe('새제목'); });
    ...
    it('디렉토리 선택 후 이동 버튼 클릭 시 PUT API를 호출한다', async () => {
        ...
        await waitFor(() => {
            expect(mockJsonPutReq).toHaveBeenCalledWith(
                '/books/101', expect.objectContaining({ category: '2_science' }), expect.any(Function), expect.any(Function)
            );
        });
    });
    ...
    it('URL 파라미터의 카테고리가 트리에 없으면 API에서 직접 조회한다', async () => {
        ...
        useSearchParams.mockReturnValue([new URLSearchParams('category=unknown_category')]);

        mockJsonGetReq.mockImplementation((url, payload, resolve, reject) => {
            if (url === '/categories') resolve(CATEGORIES);
            else if (url.startsWith('/categories/')) resolve(BOOKS_IN_FICTION);
            else if (url === '/books/777') {
                resolve({ book_id: 777, title: '직접조회책', author: '저자X', file_type: 'pdf', file_path: 'unknown_category/직접조회책.pdf', category: 'unknown_category' });
            } else resolve({});
        });
    ...
    it('URL 파라미터로 직접 조회 실패 시 에러를 설정한다', async () => {
        ...
        useSearchParams.mockReturnValue([new URLSearchParams('category=bad_category')]);
